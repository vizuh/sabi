...
async function main(): Promise<void> {
  ...
  if (jevRelevant && (argv.includes('--jev') || argv.includes('--no-jev'))) {
    ...
    // The shipped sabi.config.json ships judge.enabled: true (see setJevEnabled's own comment
    // below). Printing "off by default" without writing judge.enabled: false would leave that
    // true value in place — this branch must make the claim true, not just state it.
    if (harness !== 'hermes') {
      try {
