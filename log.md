...
## [2026-09-23] feat | spec 002 phases 2–4: receipt transitions, registry persistence, capability-gated recovery

Continued the execution evidence substrate on `feat/002-phase2-workflows` (Phase 1 — types, builders, 21 fixtures — merged via PR #123). Phase 2: an `ExecutionReceipt` now drives verification — `verificationFromExecutionReceipt` bridges a core receipt into `VerificationState` (passed/failed only; anything else reads as `invalid-receipt`, fail-closed on garbage), `strongestExecutionReceipt` selects deterministically per `operationId`, and `deriveVerificationState`/`decorateTrajectoryState`/`extractTrajectoryState` accept the receipt additively, with `'verification-receipt'` joining the reason vocabulary in type and runtime set. `packages/controller/src/registry.ts` persists core receipts by `operationId` in `execution-receipts.json` (256-entry bound, 0o600, atomic rename, changed-file names only) with a monotone merge: duplicate delivery collapses, `unknown` strengthens to a terminal verdict, and a stored terminal verdict never flips on contradiction.

Phase 3: `snapshotCapabilities` pins an untrusted declaration into a tri-state plan-time snapshot; `planRecovery` and the router consume it so receipt-dependent actions fire only on an explicit `true` — `unknown` degrades to exact 001 behavior with the plan's reason recorded. Phase 4: the planner orders `verify-local` (unverified mutation + declared `verifierReceipts`) before repair/escalation, `rollback` (repeated failure + current clean point + declared `deterministicEdit`) before the 001 reflection chain, and `switch-harness` on transport failure when a capable alternate is registered; `buildRecoveryCapsule` drops a stale (pre-compaction) clean point so it is never carried forward as a rollback bound.

Validation: `npm test` 658/658 (24 new fixtures across evidence, registry, capabilities, recovery-actions and the controller capsule; escalation-precision and 001 precedence fixtures unchanged), `npm run typecheck` clean. No paid inference, no live credentials, no commit or push — worktree-only on the phase branch.
