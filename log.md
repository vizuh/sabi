...
Operational note, learned the slow way: probing a name with `npm view` **before** it exists leaves an edge-cached 404 on the packument for roughly eight minutes after the real publish — the version document, dist-tags and visibility endpoints all resolve immediately, and nothing is wrong with the package during that window. Do not probe a name you are about to publish.
